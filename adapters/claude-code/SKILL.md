...
This file is the Claude Code adapter for the Smallest Human Action protocol. Keep the workflow portable across agent environments; do not rely on Claude-only behavior except for local skill discovery.
...
```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/update_directions.py" <repo-root> init \
...
```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/update_directions.py" <repo-root> done --result "User did the action"
```
...
```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/visualize_directions.py" <repo-root>
```
...
```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/validate_directions.py" <repo-root>
```
